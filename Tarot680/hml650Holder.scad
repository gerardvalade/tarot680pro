 ...
 */
module rh_antenna_bracket(fullView)
{
 ...
 * antenna bracket
 */
module simple_antenna_bracket(fullView)
{
	bracket_length = 70;
	bracket_width = 16;
	difference() {
		union() {
			...
			hull()
			{
				translate([0, 	bracket_length/2-6, 0]) cylinder(d=bracket_width, h=3, center=true, $fn=50);
				translate([0, 	-bracket_length/2+6, 0]) cylinder(d=bracket_width, h=3, center=true, $fn=50);
			}
		}

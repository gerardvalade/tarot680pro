...
module tarot680Modules(viewType=1, cutView=0)
{
	...
	module damperEars(type, ear_lengthx=4, damper_dia=10.5, damper_angle=40, plate_color, $fn=30)
	{
		cte=0.45;
	...
	if (viewType==FullView) {
		...
		translate([0, 0, 12]) rotate([0,0,0])  controllerPlate();
	...
	if (viewType==ViewAllModule) {
		...
		translate([120, 0, 0]) rotate([0,0,90])  controllerPlate();
		translate([240, 0, 0]) rotate([0,0,90])  controllerPlate(0);
		translate([0, 0, 0]) rotate([0,0,90])  bottomPlate();

		translate([180, -80, 0]) rotate([0,0,0])  videoBracket();
